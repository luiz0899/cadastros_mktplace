	...
	public void removerPor (int id) {
		...
		if(id > 0 ) {
			...
			if (qtdeRestaurante > 0 ) {
				throw new IllegalArgumentException("Não foi possível excluir a categoria . Motivo : Existe (m)" + qtdeRestaurante + "vinculados ");
			}
	...
	private void validar (Categoria categoria) {
		...
		if (categoria != null) {
			...
			boolean isNomeInvalido = categoria.getNome() == null
					...
					if (isNomeInvalido) {
						throw new IllegalArgumentException(" o nome da categoria é obrigatorio e " 
															+"não deve possuir mais de 100 caracters");
					}
